# css/css-grid/grid-lanes/tentative/alignment/grid-lanes-justify-content-003-ref.html

```json
{
  "format_version": 3,
  "file": "css/css-grid/grid-lanes/tentative/alignment/grid-lanes-justify-content-003-ref.html"
}
```

## style[0]

```css

html,body {
  color:black; background-color:white; font:15px/1 monospace; padding:0; margin:0;
}

grid {
  display: inline-grid;
  gap: 1px 2px;
  grid-template-columns: 1ch auto;
  grid-template-rows: repeat(4,20px);
  background: content-box silver;
  border: 1px solid;
  padding: 2px;
  width: 100px;
  justify-items: start;
}

item {
  background-color: #444;
  color: #fff;
}

.tall { grid-column:span 2; padding-left: 30px; }

.safe {
  width: 10px;
  margin-right: 50px;
}
```

```json
{
  "errors": 0,
  "messages": [],
  "warnings": 0
}
```
